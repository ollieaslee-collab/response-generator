...
app = Flask(__name__)
CORS(app, origins=["https://nomadictrails.com", "https://www.nomadictrails.com"])  # Allow requests from your WordPress domain
...
def detect_intent_from_text(text):
    """Automatically determine tour type from email content."""
    text = text.lower()
    if any(kw in text for kw in ['festival', 'naadam', 'eagle festival', 'ice festival', 'golden eagle festival']):
        return 'Festival oriented'
    if any(kw in text for kw in ['adventure', 'trek', 'hike', 'horse riding', 'archery', 'off-road', 'jeep', 'camping']):
        return 'Adventure'
    if any(kw in text for kw in ['cultural', 'shaman', 'monastery', 'history', 'museum', 'tradition', 'nomadic family']):
        return 'Cultural'
    return 'Classic'
...
def detect_duration_from_text(text):
    """Extract duration from email, e.g. '5 days' or 'one week'."""
    import re
...
def get_duration(duration_str, email_text):
    if duration_str:
        nums = re.findall(r'\d+', str(duration_str))
        if nums:
            return nums[0]
    auto_dur = detect_duration_from_text(email_text)
    if auto_dur:
        return auto_dur
    return "7"

def extract_timing(timing_str):
    return str(timing_str) if timing_str else "your preferred dates"
...
def generate_response(inquiry_dict):
    ...
    Generate response from a dictionary containing:
      - Email Request (string) [required]
      - Destination (string) [optional]
      - Duration (string) [optional]
      - Tour Timing (string) [optional]
      - Group Size (string) [optional]
      - Complexity (string) [optional]
      - Intent (string) [optional, will auto‑detect if missing]
    """
    email_text = inquiry_dict.get('Email Request', '')

    # Auto‑detect intent if not provided or empty
    intent = inquiry_dict.get('Intent', '')
    if not intent:
        intent = detect_intent_from_text(email_text)

    # Duration: prefer user input, then auto‑detect from email, then default 7
    duration = get_duration(inquiry_dict.get('Duration', ''), email_text)
...
def generate():
    ...
    # Provide defaults for optional fields (Intent will be auto‑detected)
    defaults = {
        'Intent': '',
        'Group Size': '',
        'Destination': '',
